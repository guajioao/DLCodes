...
# 删除第一个特征id（因为不携带用于预测的信息）
all_features = pd.concat((train_data.iloc[:, 1:-1], test_data.iloc[:, 1:]))


# 假设1all_features.dtypes如果不是object，那就是数值
# 提取出数值特征
...
# 利用k-折交叉验证调整超参数，再使用完整数据集训练
# 5-折验证：平均训练log rmse: 0.158970, 平均验证log rmse: 0.167784
